...
class Visualisation {
     ...
     * @returns {Array} Returns an array, the first element of which is the start year, the second element of which is the last year.
     */
    getYearRange () {
    ...
    startVis (dat) { // Function to initialise the visualisation
        if (this.syear === undefined || this.eyear === undefined) {
            console.log('Data range not defined, try calling setYearRange() first.');
            return;
        }

        if (this.lowtag === undefined || this.hightag === undefined) {
            this.setLegendTags();
        }
     ...
     */
    updateVis (year) {
        if (year > this.eyear || year < this.syear) {
            return;
        }
